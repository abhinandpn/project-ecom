...
func InitializeAPI(cfg config.Config) (*http.ServerHTTP, error) {
	...
	orderRepository := repository.NewOrderRepository(gormDB, userRepository)
	...
	orderUseCase := usecase.NewOrderUseCase(orderRepository, userRepository, productRepository, cartrepository, paymentRepository, couponrepository)
	adminHandler := handler.NewAdminHandler(adminUseCase, orderUseCase)
	productuseCase := usecase.NewProductUseCase(productRepository)
	productHandler := handler.NewProductHandler(productuseCase)
	cartUseCase := usecase.NewCartUseCase(cartrepository, productRepository, couponrepository)
